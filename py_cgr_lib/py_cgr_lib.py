...
# load contact plan from a list with the format:
# [start, end, frm, to, rate, owlt]
def cp_load_from_list(contacts, max_contacts=None):
    __contact_plan = []
    nodes = set()
    for contact in contacts:
        start, end, frm, to, rate, owlt = map(float, contact)    
        nodes.add(frm)
        nodes.add(to)
        __contact_plan.append(
            Contact(start=start, end=end, frm=frm, to=to, rate=rate, owlt=owlt))
